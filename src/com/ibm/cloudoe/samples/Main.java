...
public class Main extends HttpServlet {
		...
		if(ServletFileUpload.isMultipartContent(request)){
            try {
                ...
                for(FileItem item : multiparts){
                    if(!item.isFormField()){
                        item.write( new File(classLoader.getResource("Input").getFile()));
                    }
                }
	...
	//	}
		...
		/* chop up and tag all of our articles. */
			try {
			    NewsRepoReader reader = new NewsRepoReader(classLoader.getResource("Input").getFile());
		...
		request.getRequestDispatcher("KDMT9.jsp").forward(request, resp); 
	}
}
